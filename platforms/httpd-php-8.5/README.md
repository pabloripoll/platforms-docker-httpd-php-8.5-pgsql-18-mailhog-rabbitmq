...
COMPOSE_PROJECT_IMGK="alpine3.24-httpd-php8.5"          # <- real main image keys to manage automations for sharing resources -------------------------------> #
...
- `./docker/config/httpd/conf.d-sample/default.conf` -> `./docker/config/httpd/conf.d/default.conf`

### PHP

These are the default PHP settings. Copy them all with the proper configuration required for your project

- `./docker/config/httpd/conf.d-sample/fpm-pool.conf` -> `./docker/config/httpd/conf.d/fpm-pool.conf`
...
`./platform/httpd-php-8.5/docker/config/supervisor/conf.d-sample`

Before building the container you must copy that directory to:
`./platform/httpd-php-8.5/docker/config/supervisor/conf.d`

Make sure the copied conf.d contains at least the service files needed to run Apache and PHP-FPM (for example, supervisor program entries for Apache and php-fpm).

```bash
$ cd ./platform/httpd-php-8.5/docker/config/supervisor
$ cp -vn conf.d-sample/httpd.conf conf.d-sample/php-fpm.conf conf.d/
'conf.d-sample/httpd.conf' -> 'conf.d/httpd.conf'
'conf.d-sample/php-fpm.conf' -> 'conf.d/php-fpm.conf'
```

This approach lets developers run additional worker processes locally without changing the shared platform settings. If you need to update Apache, PHP, or supervisord configurations on a running container, there are Makefile recipes in `./platform/httpd-php-8.5/Makefile` that can apply changes *(reload or update services)* without destroying and rebuilding the container. Check the Makefile for available targets and usage by executing `$ make help` in its directory.
<br><br>
...
$ make apache-conf                    shows apache configuration set on the running container
$ make apache-update                  updates apache configuration without the need of stoping or rebuilding the container
$ make apache-default-conf            shows apache default server block set on the running container
$ make apache-default-update          updates apache default server block without the need of stoping or rebuilding the container
